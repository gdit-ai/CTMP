...
def select_model(args, device):
    ...
    if model_id == 0:
        ...
        def load_lightning_checkpoint(model, checkpoint_path, map_location=None):
            checkpoint = torch.load(checkpoint_path, map_location=map_location)
            if "state_dict" in checkpoint:
                state_dict = checkpoint["state_dict"]
            else:
                raise KeyError("can not find state_dict")

            # 移除前缀（如 'net.' 或 'model.'）
            new_state_dict = {}
            for key, value in state_dict.items():
                # print(f"=>{key}")
                if key.startswith("net.") or key.startswith("model."):
                    new_key = key[4:]
                else:
                    new_key = key
        ...
        from models.team05_CTMP import CTMP
        model_path = os.path.join('model_zoo', 'team05_CTMP.ckpt')
        # model_path = os.path.join('model_zoo', 'epoch=5-step=54000_dual_cnn.ckpt')
        model = CTMP(decoder=True)
        # 加载权重
        model = load_lightning_checkpoint(model, model_path, map_location=torch.device("cuda"))
        name, data_range = f"{model_id:02}_CTMP", 1.0
        tile = 128
